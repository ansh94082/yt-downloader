...
const FILTERS = [
  { key: "all", title: "All", icon: Download },
  { key: "downloading", title: "Downloading", icon: Activity },
  { key: "queued", title: "Queued", icon: Clock3 },
  { key: "finished", title: "Finished", icon: CheckCircle2 },
  { key: "failed", title: "Failed", icon: XCircle },
];
...
function Downloads() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.onJobsChanged((data) => {
      if (active) {
        setJobs(data || []);
      }
    });
  ...
  const filteredJobs = useMemo(() => {
    ...
    if (filter !== "all") {
      switch (filter) {
        case "downloading":
          arr = arr.filter((job) => job.status === "downloading" || job.status === "paused");
          break;
        case "queued":
          arr = arr.filter((job) => job.status === "queued");
          break;
        default:
          arr = arr.filter((job) => job.status === filter);
      }
    }
